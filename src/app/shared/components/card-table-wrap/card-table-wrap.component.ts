...
import {
  AntTableComponentToken,
  TableHeader,
} from '../ant-table/ant-table.component';
...
export class CardTableWrapComponent implements AfterContentInit {
  ...
  @ContentChild(AntTableComponentToken)
  antTableComponent!: AntTableComponentToken;
  @ContentChild(AntTreeTableComponentToken)
  antTreeTableComponent!: AntTreeTableComponentToken;
  ...
  tableSizeMenuClick(item: TableSizeItem): void {
    this.tableSizeOptions.forEach(
      (tableSizeItem) => (tableSizeItem.selected = false)
    );
    item.selected = true;
  ...
  changeTableConfigShow(): void {
    ...
    this.currentTableComponent.tableConfig.headers = [
      ...fixedLeftArray,
      ...noFixedArray,
      ...fixedRightArray,
    ];
  ...
  judgeAllChecked(): void {
    this.allTableFieldChecked = this.tableHeaders.every(
      (item) => item.show === true
    );
    const allUnChecked = this.tableHeaders.every((item) => !item.show);
    this.allTableFieldIndeterminate =
      !this.allTableFieldChecked && !allUnChecked;
  }
  ...
  ngAfterContentInit(): void {
    this.currentTableComponent =
      this.antTableComponent || this.antTreeTableComponent;
