...
import {
  ...
  SortFile,
  TableHeader,
} from '@shared/components/ant-table/ant-table.component';
import {
...
export class TreeTableComponent implements OnChanges {
  ...
  @Output() readonly sortFn: EventEmitter<SortFile> =
    new EventEmitter<SortFile>();
  ...
  @Output() readonly selectedChange: EventEmitter<NzSafeAny[]> =
    new EventEmitter<NzSafeAny[]>();
  ...
  changeSort(tableHeader: TableHeader): void {
    ...
    const index = sortDicArray.findIndex(
      (item) => item === tableHeader.sortDir
    );
    tableHeader.sortDir =
      index === sortDicArray.length - 1
        ? sortDicArray[0]
        : sortDicArray[index + 1];
    this.sortFn.emit({
      fileName: tableHeader.field!,
      sortDir: tableHeader.sortDir,
    });
  }
  ...
  setIsCheckFn(dataItem: NzSafeAny, isChecked: boolean): void {
    ...
    const index = this.checkedCashArrayFromComment.findIndex(
      (cashItem) => cashItem.id === dataItem.id
    );
    if (isChecked) {
      if (index === -1) {
  ...
  // 单选
  public checkRowSingle(
    isChecked: boolean,
    selectIndex: number,
    row: TreeNodeInterface
  ): void {
    this.setIsCheckFn(row, isChecked);
    this.selectedChange.emit(this.checkedCashArrayFromComment);
    this.refreshStatus();
  ...
  refreshStatus(): void {
    // 获取铺平的treeData
    const dataTempArray: TreeNodeInterface[] = fnGetFlattenTreeDataByMap(
    ...
    const allUnChecked =
      dataTempArray.length > 0 &&
      dataTempArray.every((item) => item['_checked'] !== true);
